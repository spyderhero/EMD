...
subroutine GibbonsMaeda_calc_rhs( CCTK_ARGUMENTS )
  ...
  do i = 1+cctk_nghostzones(1), cctk_lsh(1)-cctk_nghostzones(1)
    ...
    do a = 1, 3
       do b = 1, 3
          rhs_lKphi1 = rhs_lKphi1 - ch**conf_fac_exponent * hu(a,b) * cd2_lphi1(a,b)        & 
                     - 0.50d0 * hu(a,b) * conf_fac_exponent * ch**(conf_fac_exponent-1)     &
                                                            * d1_lphi1(a) * d1_ch(b)        &
                     - EXP(-2 * lphi1) * hh(a,b) * lE(a) * lE(b) / ch                       &
                     - ch**conf_fac_exponent * hu(a,b) * d1_lphi1(a) * d1_alph(b)
          do c = 1, 3
             do m = 1, 3
                rhs_lKphi1 = rhs_lKphi1                                                     &
                           + EXP(-2 * lphi1) * ch**(2*conf_fac_exponent)                    &
                           * hu(a,b) * hu(c,m) * d1_lA(b,m) * ( d1_lA(c,a) - d1_lA(a,c) )
             end do
          end do
       end do
    end do
