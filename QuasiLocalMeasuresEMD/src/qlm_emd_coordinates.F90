...
subroutine qlm_emd_calc_coordinates (CCTK_ARGUMENTS, hn)
  use cctk
  use constants
  ...
  do j = 1+qlm_emd_nghostsphi(hn), qlm_emd_nphi(hn)-qlm_emd_nghostsphi(hn)
     do i = 1+qlm_emd_nghoststheta(hn), qlm_emd_ntheta(hn)-qlm_emd_nghoststheta(hn)
        ...
        area = area + sqrt(dtq) * qlm_emd_delta_theta(hn) * qlm_emd_delta_phi(hn)
  ...
  ! initial value
  qlm_emd_inv_z(i0,j0,hn) = 0
  
  ! d_a z = 1/R^2 xi^b eps_ba
  
  ! transport along equator
  do j = j0+1, qlm_emd_nphi(hn)-qlm_emd_nghostsphi(hn)
     i = i0
     
     z0 = qlm_emd_inv_z(i,j-1,hn)
     z0dot = rhs(i,j-1,(/0,1/))
     z1 = z0 + qlm_emd_delta_phi(hn) * z0dot
     z1dot = rhs(i,j,(/0,1/))
     qlm_emd_inv_z(i,j,hn) = z0 + half * qlm_emd_delta_phi(hn) * (z0dot + z1dot)
  end do
     
  ! transport along meridians
  do j = 1+qlm_emd_nghostsphi(hn), qlm_emd_nphi(hn)-qlm_emd_nghostsphi(hn)
     do i = i0-1, 1+qlm_emd_nghoststheta(hn), -1
        z0 = qlm_emd_inv_z(i+1,j,hn)
        z0dot = rhs(i+1,j,(/-1,0/))
        z1 = z0 + qlm_emd_delta_theta(hn) * z0dot
        z1dot = rhs(i,j,(/-1,0/))
        qlm_emd_inv_z(i,j,hn) = z0 + half * qlm_emd_delta_theta(hn) * (z0dot + z1dot)
     end do
     
     do i = i0+1, qlm_emd_ntheta(hn)-qlm_emd_nghoststheta(hn)
        z0 = qlm_emd_inv_z(i-1,j,hn)
        z0dot = rhs(i-1,j,(/1,0/))
        z1 = z0 + qlm_emd_delta_theta(hn) * z0dot
        z1dot = rhs(i,j,(/1,0/))
  ...
  do j = 1+qlm_emd_nghostsphi(hn), qlm_emd_nphi(hn)-qlm_emd_nghostsphi(hn)
     do i = 1+qlm_emd_nghoststheta(hn), qlm_emd_ntheta(hn)-qlm_emd_nghoststheta(hn)
        ...
        integral_z = integral_z + qlm_emd_inv_z(i,j,hn) * sqrt(dtq) * qlm_emd_delta_theta(hn) * qlm_emd_delta_phi(hn)
...
contains
  ...
  function rhs (i, j, vv) result (zdot)
    ...
    zdot = vv(1) * (- (1/radius**2) * qlm_emd_xi_p(i,j,hn) * sqrt(dtq)) &
         + vv(2) * (+ (1/radius**2) * qlm_emd_xi_t(i,j,hn) * sqrt(dtq))
  end function rhs
  
end subroutine qlm_emd_calc_coordinates
