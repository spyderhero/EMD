    ...
    for (int j = imin[1]; j < imax[1]; ++j) {
      for (int k = imin[2]; k < imax[2]; ++k) {
        ...
        Ax[ind]    = par_qm_plus * z1 * y1 / r_plus / (pow(x1 - par_b,2) + pow(y1,2)) 
                    + par_qm_minus * z1 * y1 / r_plus / (pow(x1 + par_b,2) + pow(y1,2))/ pow(psi1, 2);

        Ay[ind]    = - par_qm_plus * z1 * x1 / r_plus / (pow(x1 - par_b,2) + pow(y1,2)) 
                    - par_qm_minus * z1 * x1 / r_plus / (pow(x1 + par_b,2) + pow(y1,2))/ pow(psi1, 2);
