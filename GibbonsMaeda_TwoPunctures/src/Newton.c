              ...
              CCTK_REAL const * restrict const * restrict const JFD)
{
  ...
  for (ivar = 0; ivar < nvar; ivar++)
  {
    ...
    for (i = 0; i < n1; i++)
    {
      Ip = GibbonsMaeda_Index (ivar, i + 1, j, k, nvar, n1, n2, n3);
      Ic = GibbonsMaeda_Index (ivar, i, j, k, nvar, n1, n2, n3);
      Im = GibbonsMaeda_Index (ivar, i - 1, j, k, nvar, n1, n2, n3);
              ...
              CCTK_REAL const * restrict const * restrict const JFD)
{
  ...
  for (ivar = 0; ivar < nvar; ivar++)
  {
    ...
    for (j = 0; j < n2; j++)
    {
      Ip = GibbonsMaeda_Index (ivar, i, j + 1, k, nvar, n1, n2, n3);
      Ic = GibbonsMaeda_Index (ivar, i, j, k, nvar, n1, n2, n3);
      Im = GibbonsMaeda_Index (ivar, i, j - 1, k, nvar, n1, n2, n3);
...
/* --------------------------------------------------------------------------*/
void
GibbonsMaeda_TestRelax (CCTK_POINTER_TO_CONST cctkGH,
	   ...
	   CCTK_REAL *dv)
{
  ...
  GibbonsMaeda_SetMatrix_JFD (nvar, n1, n2, n3, u, ncols, cols, JFD);
          ...
          CCTK_REAL * restrict const normres)
{
  ...
  GibbonsMaeda_allocate_derivs (&ph, ntotal);
/*      ph  = dvector(0, ntotal-1);*/
  rt = dvector (0, ntotal - 1);
  s = dvector (0, ntotal - 1);
  GibbonsMaeda_allocate_derivs (&sh, ntotal);
/*      sh  = dvector(0, ntotal-1);*/
  ...
  GibbonsMaeda_J_times_dv (nvar, n1, n2, n3, dv, r, u);
#pragma omp parallel for
  for (int j = 0; j < ntotal; j++)
    rt[j] = r[j] = F[j] - r[j];
...
#pragma omp parallel for
        ...
        GibbonsMaeda_J_times_dv (nvar, n1, n2, n3, ph, vv, u);	/* vv=J*ph*/
...
#pragma omp parallel for
        ...
        GibbonsMaeda_J_times_dv (nvar, n1, n2, n3, sh, t, u);	/* t=J*sh*/
...
/* -------------------------------------------------------------------*/
void
GibbonsMaeda_Newton (CCTK_POINTER_TO_CONST const cctkGH,
        int const nvar, int const n1, int const n2, int const n3,
	derivs v,
