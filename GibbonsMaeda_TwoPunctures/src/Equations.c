...
CCTK_REAL
GibbonsMaeda_BY_KKofxyz (CCTK_REAL x, CCTK_REAL y, CCTK_REAL z)
{
...
void
GibbonsMaeda_BY_Aijofxyz (CCTK_REAL x, CCTK_REAL y, CCTK_REAL z, CCTK_REAL Aij[3][3])
{
  DECLARE_CCTK_PARAMETERS;
  int i, j;
  CCTK_REAL r_plus, r2_plus, r3_plus, r_minus, r2_minus, r3_minus, np_Pp, nm_Pm,
...
/*-----------------------------------------------------------*/
void
GibbonsMaeda_Calc_E(CCTK_REAL x1, CCTK_REAL y1, CCTK_REAL z1, 
...
/*-----------------------------------------------------------*/
void
GibbonsMaeda_NonLinEquations (CCTK_REAL rho_adm,
		 ...
		 CCTK_REAL y, CCTK_REAL z, derivs U, CCTK_REAL *values)
{
  ...
  psi = sqrt( pow(1 + 0.5 * par_m_plus / r_plus
              ...
              + par_q_minus/r_minus, 2) / exp(2 * coupling_constant * phi1_0) ) + U.d0[0];
  psi_ana = sqrt( pow(1 + 0.5 * par_m_plus / r_plus
              + 0.5 * par_m_minus/ r_minus , 2)
              - 0.25 * pow( par_q_plus/r_plus
              + par_q_minus/r_minus, 2) / exp(2 * coupling_constant * phi1_0) );
  psi2 = psi * psi;
  psi4 = psi2 * psi2;
  psi7 = psi * psi2 * psi4;
  psi_ana2 = psi_ana * psi_ana;

  GibbonsMaeda_Calc_E(x, y, z, Ei, &E2);
...
/*-----------------------------------------------------------*/
void
GibbonsMaeda_LinEquations (CCTK_REAL A, CCTK_REAL B, CCTK_REAL X, CCTK_REAL R,
	      CCTK_REAL x, CCTK_REAL r, CCTK_REAL phi,
	      CCTK_REAL y, CCTK_REAL z, derivs dU, derivs U, CCTK_REAL *values)
{
  ...
  values[0] = dU.d11[0] + dU.d22[0] + dU.d33[0]
    - 0.875 * GibbonsMaeda_BY_KKofxyz (x, y, z) / psi8 * dU.d0[0]
    - 0.75 * E2 / psi4 / exp(2 * coupling_constant * phi1_0) * dU.d0[0];
}
