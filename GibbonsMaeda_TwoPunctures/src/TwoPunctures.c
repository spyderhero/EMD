                       ...
                       derivs v)
{
  ...
  for (ivar = 0; ivar < nvar; ivar++)
    for (i = 0; i < n1; i++)
      for (j = 0; j < n2; j++)
        for (k = 0; k < n3; k++)
        {
          i3D = GibbonsMaeda_Index(ivar,i,j,k,1,n1,n2,n3);
  ...
  for (ivar = 0; ivar < nvar; ivar++)
    for (i = 0; i < n1; i++)
      for (j = 0; j < n2; j++)
        for (k = 0; k < n3; k++)
        {
          indx = GibbonsMaeda_Index(ivar,i,j,k,1,n1,n2,n3);
  ...
  if (do_initial_debug_output && CCTK_MyProc(cctkGH) == 0)
  {
    ...
    for (ivar = 0; ivar < nvar; ivar++)
      for (i = 0; i < n1; i++)
        for (j = 0; j < n2; j++)
        {
          ...
          B = -cos (be);
          phi = 0.0;
          indx = GibbonsMaeda_Index(ivar,i,j,0,1,n1,n2,n3);
        ...
        GibbonsMaeda_AB_To_XR (nvar, A, B, &X, &R, U);
        /* Calculation of (x,r)*/
        GibbonsMaeda_C_To_c (nvar, X, R, &(s_x[indx]), &r, U);
        /* Calculation of (y,z)*/
        GibbonsMaeda_rx3_To_xyz (nvar, s_x[i3D], r, phi, &(s_y[indx]), &(s_z[indx]), U);
        fprintf(debug_file,
    ...
    {
      CCTK_REAL d;
      indx = GibbonsMaeda_Index(0,0,i,0,1,n1,n2,n3);
      d = GibbonsMaeda_PunctIntPolAtArbitPosition(0, nvar, n1, n2, n3, v,
              s_x[indx], 0.0, 0.0);
    ...
    {
      CCTK_REAL d;
      int ip= GibbonsMaeda_Index(0,0,i+1,0,1,n1,n2,n3);
      indx = GibbonsMaeda_Index(0,0,i,0,1,n1,n2,n3);
      for (j=-10; j<10; j++)
      {
        d = GibbonsMaeda_PunctIntPolAtArbitPosition(0, nvar, n1, n2, n3, v,
...
/* -------------------------------------------------------------------*/
void GibbonsMaeda_TwoPunctures(CCTK_ARGUMENTS);
void
GibbonsMaeda_TwoPunctures (CCTK_ARGUMENTS)
{
  ...
  * mm = par_m_minus;
  ...
  if (! F) {
    ...
    GibbonsMaeda_allocate_derivs (&u, ntotal);
    GibbonsMaeda_allocate_derivs (&v, ntotal);
    GibbonsMaeda_allocate_derivs (&cf_v, ntotal);
    ...
    if(!(give_bare_mass)) {
      ...
      do {
        CCTK_VInfo (CCTK_THORNSTRING, "Bare masses: mp=%.15g, mm=%.15g",
                    (double)*mp, (double)*mm);
        GibbonsMaeda_Newton (cctkGH, nvar, n1, n2, n3, v, Newton_tol, 1);

        GibbonsMaeda_F_of_v (cctkGH, nvar, n1, n2, n3, v, F, u);

        up = GibbonsMaeda_PunctIntPolAtArbitPosition(0, nvar, n1, n2, n3, v, par_b, 0., 0.);
        um = GibbonsMaeda_PunctIntPolAtArbitPosition(0, nvar, n1, n2, n3, v,-par_b, 0., 0.);
    ...
    GibbonsMaeda_Newton (cctkGH, nvar, n1, n2, n3, v, Newton_tol, Newton_maxit);
 
    GibbonsMaeda_F_of_v (cctkGH, nvar, n1, n2, n3, v, F, u);

    GibbonsMaeda_SpecCoef(n1, n2, n3, nvar, v.d0, cf_v.d0);
    ...
    *mm_adm = (1 + um) * *mm + *mp * *mm / (4. * par_b);
    ...
    admMass = (*mp + *mm
               - 4*par_b*GibbonsMaeda_PunctEvalAtArbitPosition(v.d0, 0, 1, 0, 0, nvar, n1, n2, n3));
  ...
  for (int k = imin[2]; k < imax[2]; ++k)
  {
    for (int j = imin[1]; j < imax[1]; ++j)
    {
      for (int i = imin[0]; i < imax[0]; ++i)
      {
        ...
        CCTK_REAL Ei[3], E2;
        GibbonsMaeda_Calc_E(xx, yy, zz, Ei, &E2);

        CCTK_REAL U;
        switch (gsm)
        {
        case GSM_Taylor_expansion:
          U = GibbonsMaeda_PunctTaylorExpandAtArbitPosition
            (0, nvar, n1, n2, n3, v, xx, yy, zz);
          break;
        case GSM_evaluation:
          U = GibbonsMaeda_PunctIntPolAtArbitPositionFast(0, nvar, n1, n2, n3, cf_v, xx, yy, zz);
          break;
        default:
          assert (0);
        }
        ...
        CCTK_REAL psi1 = sqrt( pow(1 + 0.5 * *mp / r_plus
              ...
              + par_q_minus/r_minus, 2) / exp(2 * coupling_constant * phi1_0) ) + U;
#define EXTEND(M,r) \
        ...
        CCTK_REAL Aij[3][3];
        GibbonsMaeda_BY_Aijofxyz (xx, yy, zz, Aij);
  ...
  if (0) {
    ...
    GibbonsMaeda_free_derivs (&u, ntotal);
    GibbonsMaeda_free_derivs (&v, ntotal);
    GibbonsMaeda_free_derivs (&cf_v, ntotal);
  }
}
