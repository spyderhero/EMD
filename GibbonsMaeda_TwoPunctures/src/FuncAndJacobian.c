...
/* --------------------------------------------------------------------------*/
int
GibbonsMaeda_Index (int ivar, int i, int j, int k, int nvar, int n1, int n2, int n3)
{
  int i1 = i, j1 = j, k1 = k;
...
/* --------------------------------------------------------------------------*/
void
GibbonsMaeda_allocate_derivs (derivs * v, int n)
{
  int m = n - 1;
  (*v).d0 = dvector (0, m);
  (*v).d1 = dvector (0, m);
...
/* --------------------------------------------------------------------------*/
void
GibbonsMaeda_free_derivs (derivs * v, int n)
{
  int m = n - 1;
...
/* --------------------------------------------------------------------------*/
void
GibbonsMaeda_Derivatives_AB3 (int nvar, int n1, int n2, int n3, derivs v)
{
  ...
  for (ivar = 0; ivar < nvar; ivar++)
  {
	...
	for (i = 0; i < n1; i++)
	{
	  indx[i] = GibbonsMaeda_Index (ivar, i, j, k, nvar, n1, n2, n3);
	  p[i] = v.d0[indx[i]];
	}
	chebft_Zeros (p, n1, 0);
	chder (p, dp, n1);
	...
	for (j = 0; j < n2; j++)
	{
	  indx[j] = GibbonsMaeda_Index (ivar, i, j, k, nvar, n1, n2, n3);
	  p[j] = v.d0[indx[j]];
	  q[j] = v.d1[indx[j]];
	}
	chebft_Zeros (p, n2, 0);
	...
	for (k = 0; k < n3; k++)
	{
	  indx[k] = GibbonsMaeda_Index (ivar, i, j, k, nvar, n1, n2, n3);
	  p[k] = v.d0[indx[k]];
	  q[k] = v.d1[indx[k]];
	  r[k] = v.d2[indx[k]];
	}
...
/* --------------------------------------------------------------------------*/
void
GibbonsMaeda_F_of_v (CCTK_POINTER_TO_CONST cctkGH,
        ...
        derivs u)
{
  ...
  GibbonsMaeda_allocate_derivs (&U, nvar);
  ...
  if (use_sources)
  {
    ...
    for (i = 0; i < n1; i++)
      for (j = 0; j < n2; j++)
        for (k = 0; k < n3; k++)
        {
          i3D = GibbonsMaeda_Index(0,i,j,k,1,n1,n2,n3);
          ...
          for (ivar = 0; ivar < nvar; ivar++)
          {
            indx = GibbonsMaeda_Index (ivar, i, j, k, nvar, n1, n2, n3);
          ...
          GibbonsMaeda_AB_To_XR (nvar, A, B, &X, &R, U);
          /* Calculation of (x,r) and*/
          /* (U, U_x, U_r, U_3, U_xx, U_xr, U_x3, U_rr, U_r3, U_33)*/
          GibbonsMaeda_C_To_c (nvar, X, R, &(s_x[i3D]), &r, U);
          /* Calculation of (y,z) and*/
          /* (U, U_x, U_y, U_z, U_xx, U_xy, U_xz, U_yy, U_yz, U_zz)*/
          GibbonsMaeda_rx3_To_xyz (nvar, s_x[i3D], r, phi, &(s_y[i3D]), &(s_z[i3D]), U);
        }
    ...
    for (i = 0; i < n1; i++)
      for (j = 0; j < n2; j++)
        for (k = 0; k < n3; k++)
          sources[GibbonsMaeda_Index(0,i,j,k,1,n1,n2,n3)]=0.0;

  GibbonsMaeda_Derivatives_AB3 (nvar, n1, n2, n3, v);
  ...
  for (i = 0; i < n1; i++)
  {
    for (j = 0; j < n2; j++)
    {
      for (k = 0; k < n3; k++)
      {
        ...
        /* Calculation of (x,r) and*/
        /* (U, U_x, U_r, U_3, U_xx, U_xr, U_x3, U_rr, U_r3, U_33)*/
        GibbonsMaeda_C_To_c (nvar, X, R, &x, &r, U);
        /* Calculation of (y,z) and*/
        /* (U, U_x, U_y, U_z, U_xx, U_xy, U_xz, U_yy, U_yz, U_zz)*/
        GibbonsMaeda_rx3_To_xyz (nvar, x, r, phi, &y, &z, U);
        GibbonsMaeda_NonLinEquations (sources[GibbonsMaeda_Index(0,i,j,k,1,n1,n2,n3)],
        ...
        if (debugfile && (k==0))
        {
          ...
          psi = sqrt( pow(1 + 0.5 * par_m_plus / r_plus
              ...
              + par_q_minus/r_minus, 2) / exp(4 * coupling_constant * phi1_0) ) +
                U.d0[0];
          psi2 = psi * psi;
          psi4 = psi2 * psi2;
          psi7 = psi * psi2 * psi4;
...
/*                      0.125 * BY_KKofxyz (x, y, z) / psi7 +*/
  ...
  GibbonsMaeda_free_derivs (&U, nvar);
}

/* --------------------------------------------------------------------------*/
void
GibbonsMaeda_J_times_dv (int nvar, int n1, int n2, int n3, derivs dv,
...
#pragma omp parallel for private (values,dU,U,i,j,k,al,A,be,B,phi,X,R,x,r,y,z,Am1,ivar,indx) schedule(dynamic)
  for (i = 0; i < n1; i++)
  {
	...
	GibbonsMaeda_AB_To_XR (nvar, A, B, &X, &R, dU);
	/* Calculation of (x,r) and*/
	/* (dU, dU_x, dU_r, dU_3, dU_xx, dU_xr, dU_x3, dU_rr, dU_r3, dU_33)*/
	GibbonsMaeda_C_To_c (nvar, X, R, &x, &r, dU);
	/* Calculation of (y,z) and*/
	/* (dU, dU_x, dU_y, dU_z, dU_xx, dU_xy, dU_xz, dU_yy, dU_yz, dU_zz)*/
	GibbonsMaeda_rx3_To_xyz (nvar, x, r, phi, &y, &z, dU);
	GibbonsMaeda_LinEquations (A, B, X, R, x, r, phi, y, z, dU, U, values);
	for (ivar = 0; ivar < nvar; ivar++)
	{
	  indx = GibbonsMaeda_Index (ivar, i, j, k, nvar, n1, n2, n3);
    ...
    free_dvector (values, 0, nvar - 1);
    GibbonsMaeda_free_derivs (&dU, nvar);
    GibbonsMaeda_free_derivs (&U, nvar);
  }
}
...
/* --------------------------------------------------------------------------*/
void
GibbonsMaeda_JFD_times_dv (int i, int j, int k, int nvar, int n1, int n2,
	      int n3, derivs dv, derivs u, CCTK_REAL *values)
{				/* Calculates rows of the vector 'J(FD)*dv'.*/
  /* First row to be calculated: row = GibbonsMaeda_Index(0,      i, j, k; nvar, n1, n2, n3)*/
  /* Last  row to be calculated: row = GibbonsMaeda_Index(nvar-1, i, j, k; nvar, n1, n2, n3)*/
  ...
  for (ivar = 0; ivar < nvar; ivar++)
  {
    int iccc = GibbonsMaeda_Index (ivar, i, j, k, nvar, n1, n2, n3),
      ipcc = GibbonsMaeda_Index (ivar, i + 1, j, k, nvar, n1, n2, n3),
      imcc = GibbonsMaeda_Index (ivar, i - 1, j, k, nvar, n1, n2, n3),
      icpc = GibbonsMaeda_Index (ivar, i, j + 1, k, nvar, n1, n2, n3),
      icmc = GibbonsMaeda_Index (ivar, i, j - 1, k, nvar, n1, n2, n3),
      iccp = GibbonsMaeda_Index (ivar, i, j, k + 1, nvar, n1, n2, n3),
      iccm = GibbonsMaeda_Index (ivar, i, j, k - 1, nvar, n1, n2, n3),
      icpp = GibbonsMaeda_Index (ivar, i, j + 1, k + 1, nvar, n1, n2, n3),
      icmp = GibbonsMaeda_Index (ivar, i, j - 1, k + 1, nvar, n1, n2, n3),
      icpm = GibbonsMaeda_Index (ivar, i, j + 1, k - 1, nvar, n1, n2, n3),
      icmm = GibbonsMaeda_Index (ivar, i, j - 1, k - 1, nvar, n1, n2, n3),
      ipcp = GibbonsMaeda_Index (ivar, i + 1, j, k + 1, nvar, n1, n2, n3),
      imcp = GibbonsMaeda_Index (ivar, i - 1, j, k + 1, nvar, n1, n2, n3),
      ipcm = GibbonsMaeda_Index (ivar, i + 1, j, k - 1, nvar, n1, n2, n3),
      imcm = GibbonsMaeda_Index (ivar, i - 1, j, k - 1, nvar, n1, n2, n3),
      ippc = GibbonsMaeda_Index (ivar, i + 1, j + 1, k, nvar, n1, n2, n3),
      impc = GibbonsMaeda_Index (ivar, i - 1, j + 1, k, nvar, n1, n2, n3),
      ipmc = GibbonsMaeda_Index (ivar, i + 1, j - 1, k, nvar, n1, n2, n3),
      immc = GibbonsMaeda_Index (ivar, i - 1, j - 1, k, nvar, n1, n2, n3);
...
/* --------------------------------------------------------------------------*/
void
GibbonsMaeda_SetMatrix_JFD (int nvar, int n1, int n2, int n3, derivs u,
	       int *ncols, int **cols, CCTK_REAL **Matrix)
{
  ...
  values = dvector (0, nvar - 1);
  GibbonsMaeda_allocate_derivs (&dv, ntotal);
...
#pragma omp parallel for private (i,j,k,ivar,row) schedule(dynamic)
  for (i = 0; i < n1; i++)
  {
	...
	for (ivar = 0; ivar < nvar; ivar++)
	{
	  row = GibbonsMaeda_Index (ivar, i, j, k, nvar, n1, n2, n3);
	  ncols[row] = 0;
	  dv.d0[row] = 0;
	}
      }
  ...
  for (i = 0; i < n1; i++)
  {
	...
	for (ivar = 0; ivar < nvar; ivar++)
	{
	  column = GibbonsMaeda_Index (ivar, i, j, k, nvar, n1, n2, n3);
	  dv.d0[column] = 1;
...
/*					i_0 = 0;
	  ...
	  for (i1 = i_0; i1 <= i_1; i1++)
	  {
	    for (j1 = j_0; j1 <= j_1; j1++)
	    {
	      ...
	      {
		GibbonsMaeda_JFD_times_dv (i1, j1, k1, nvar, n1, n2, n3,
			      dv, u, values);
		for (ivar1 = 0; ivar1 < nvar; ivar1++)
		{
		  if (values[ivar1] != 0)
		  {
		    row = GibbonsMaeda_Index (ivar1, i1, j1, k1, nvar, n1, n2, n3);
		    mcol = ncols[row];
  ...
  GibbonsMaeda_free_derivs (&dv, ntotal);
  free_dvector (values, 0, nvar - 1);
}
...
CCTK_REAL
GibbonsMaeda_PunctEvalAtArbitPosition (CCTK_REAL *v, int ivar, CCTK_REAL A, CCTK_REAL B, CCTK_REAL phi,
...
/* --------------------------------------------------------------------------*/
void
GibbonsMaeda_calculate_derivs (int i, int j, int k, int ivar, int nvar, int n1, int n2,
		  int n3, derivs v, derivs vv)
{
  ...
  vv.d0[0] = v.d0[GibbonsMaeda_Index (ivar, i, j, k, nvar, n1, n2, n3)];
  vv.d1[0] = v.d1[GibbonsMaeda_Index (ivar, i, j, k, nvar, n1, n2, n3)] * sin_al;
  vv.d2[0] = v.d2[GibbonsMaeda_Index (ivar, i, j, k, nvar, n1, n2, n3)] * sin_be;
  vv.d3[0] = v.d3[GibbonsMaeda_Index (ivar, i, j, k, nvar, n1, n2, n3)];
  vv.d11[0] = v.d11[GibbonsMaeda_Index (ivar, i, j, k, nvar, n1, n2, n3)] * sin2_al
    + v.d1[GibbonsMaeda_Index (ivar, i, j, k, nvar, n1, n2, n3)] * cos_al;
  vv.d12[0] =
    v.d12[GibbonsMaeda_Index (ivar, i, j, k, nvar, n1, n2, n3)] * sin_al * sin_be;
  vv.d13[0] = v.d13[GibbonsMaeda_Index (ivar, i, j, k, nvar, n1, n2, n3)] * sin_al;
  vv.d22[0] = v.d22[GibbonsMaeda_Index (ivar, i, j, k, nvar, n1, n2, n3)] * sin2_be
    + v.d2[GibbonsMaeda_Index (ivar, i, j, k, nvar, n1, n2, n3)] * cos_be;
  vv.d23[0] = v.d23[GibbonsMaeda_Index (ivar, i, j, k, nvar, n1, n2, n3)] * sin_be;
  vv.d33[0] = v.d33[GibbonsMaeda_Index (ivar, i, j, k, nvar, n1, n2, n3)];
}

/* --------------------------------------------------------------------------*/
CCTK_REAL
GibbonsMaeda_interpol (CCTK_REAL a, CCTK_REAL b, CCTK_REAL c, derivs v)
{
  return v.d0[0]
...
/* --------------------------------------------------------------------------*/
static CCTK_REAL
GibbonsMaeda_clamp_pm_one (CCTK_REAL val)
{
...
CCTK_REAL
GibbonsMaeda_PunctTaylorExpandAtArbitPosition (int ivar, int nvar, int n1,
                                  ...
                                  CCTK_REAL z)
{
  ...
  int i, j, k;
  derivs vv;
  GibbonsMaeda_allocate_derivs (&vv, 1);
  ...
  A = GibbonsMaeda_clamp_pm_one( 2 * tanh (0.5 * X) - 1 );

  /* Note: Range of R/2 - pi/4 is [ -pi/4, pi/4 ] and so range of tan
   * is [-1,1], for R in [0,pi]. */
  B = GibbonsMaeda_clamp_pm_one( tan (0.5 * R - Piq) );
  al = Pi - acos (A);
  be = Pi - acos (B);
  ...
  GibbonsMaeda_calculate_derivs (i, j, k, ivar, nvar, n1, n2, n3, v, vv);
  result = GibbonsMaeda_interpol (a, b, c, vv);
  GibbonsMaeda_free_derivs (&vv, 1);
...
/* Calculates the value of v at an arbitrary position (x,y,z)*/
CCTK_REAL
GibbonsMaeda_PunctIntPolAtArbitPosition (int ivar, int nvar, int n1,
			    int n2, int n3, derivs v, CCTK_REAL x, CCTK_REAL y,
			    CCTK_REAL z)
{
  DECLARE_CCTK_PARAMETERS;
  ...
  result = GibbonsMaeda_PunctEvalAtArbitPosition (v.d0, ivar, A, B, phi, nvar, n1, n2, n3);
...
CCTK_REAL
GibbonsMaeda_PunctEvalAtArbitPositionFast (CCTK_REAL *v, int ivar, CCTK_REAL A, CCTK_REAL B, CCTK_REAL phi, int nvar, int n1, int n2, int n3)
{
  int i, j, k, N;
  CCTK_REAL *p, *values1, **values2, result;
  // VASILIS: Nothing should be changed in this routine. This is used by GibbonsMaeda_PunctIntPolAtArbitPositionFast
...
/* --------------------------------------------------------------------------*/
CCTK_REAL
GibbonsMaeda_PunctIntPolAtArbitPositionFast (int ivar, int nvar, int n1,
			    int n2, int n3, derivs v, CCTK_REAL x, CCTK_REAL y,
			    CCTK_REAL z)
{
  DECLARE_CCTK_PARAMETERS;
  ...
  result = GibbonsMaeda_PunctEvalAtArbitPositionFast (v.d0, ivar, A, B, phi, nvar, n1, n2, n3);
...
void GibbonsMaeda_SpecCoef(int n1, int n2, int n3, int nvar, CCTK_REAL *v, CCTK_REAL *cf)
{
  DECLARE_CCTK_PARAMETERS;
